...
<ul>
<li v-for="item in categories"><a href="#" id="fashion categories" :class="navclasses">{{item.name}}<font-awesome-icon icon="caret-down" class="caret" /></a>
...
name: 'Sitenav', 
  ...
  data(){ 
  return { browserheight: 0, previousscrolly: 0, scrolldirection: null, siteheader: [], menucontent: [], menuicon:[], navclasses: [], navlogo: [], navbtn: [], mobilenav: false,  categories: data.dataCategories, products: data.dataProducts  }
},
mounted(){
...
runOnScroll() {
    ...
    if (window.scrollY > 60) {
      this.siteheader.push('site-header--dark')
	  this.navclasses.push('dark-style-links')
	  this.navlogo.push('svg__logo--dark')
	  this.navbtn.push('btn--dark','btn--white')
    } else {
      this.siteheader = []
	  this.navclasses = []
	  this.navlogo = []
